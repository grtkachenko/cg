...
namespace cg
{
   ...
   template <class Scalar>
   void update_edge(segment_2t<Scalar> & seg, std::set<segment_2t<Scalar>, edge_less<Scalar> > & border) {
   ...
   std::vector<triangle_2t<Scalar> > get_delaunay_triangulation(std::vector<point_2t<Scalar> > const & initial_points) {
      ...
      while (!border.empty()) {
         ...
         for (int i = 0; i < points.size(); i++) {
            ...
            if (e[0] != p && e[1] != p && cg::orientation(e[0], e[1], p) == cg::CG_RIGHT) {
               if ((point_by_edge.count(e) == 0 || less_ort(e, point_by_edge[e], p)) &&
                       (best_point == nullptr || less_ort(e, p, *best_point))) {
                  best_point = &points[i];
               }
            }
         }
         ...
         if (best_point) {
            Point s(e[0]), f(e[1]);
            res.push_back(Triangle(s, f, *best_point));
            Segment first(f, *best_point), second(*best_point, s);
            update_edge(first, border);
            update_edge(second, border);
            point_by_edge[first] = s;
            point_by_edge[second] = f;
         }
      }
