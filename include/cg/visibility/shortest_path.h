...
namespace cg
{
	...
	void add_point(int num, point_2t<Scalar> & p, std::map<point_2t<Scalar>, int> & number_by_point,
						std::map<int, point_2t<Scalar> > & point_by_number) {
		number_by_point[p] = num;
		point_by_number[num] = p;
	}

	std::vector<int> get_path(std::vector<std::vector<std::pair<int, double> > > & a, int start, int end, int size) {
	...
	template <class Scalar>
	std::vector<point_2t<Scalar> > get_shortest_path(point_2t<Scalar> & start, point_2t<Scalar> & end, std::vector<contour_2t<Scalar> > & polygons) {
		...
		add_point(0, start, number_by_point, point_by_number);
		int cur_point = 1;
